...
//			path = cnt != 0 ? "/join/registerok.jsp" : "/join/registerfail.jsp";
			...
			MemberDetailDto memberDetailDto = memberService
					.getMember(((MemberDto) session.getAttribute("userInfo")).getId());
...
/*
 * Session & Cookie 1.session : HttpSession Server Object(메모리할당) HttpSession
 * session = request.getSession(); // 세션 생성 session.setAttribute("name",object);
 * 
 * Object = obj = request.getAttribute("name"); request.removeAttribute("name");
 * 
 * 2.cookie : Cookie Client txt(물리적인파일)
 * 
 * Cookie cookie = new cookie("name","value"); cookie.setDomain("domain");
 * cookie.Path("path"); cookie.setMaxAge(second);
 * 
 */
